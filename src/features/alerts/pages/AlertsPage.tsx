...
export default function Alerts() {
  ...
  const sevColor = (s: string) =>
    s === "high" ? "border-l-destructive" : s === "medium" ? "border-l-warning" : "border-l-primary";
  ...
  return (
    <AppLayout>
      ...
      {loading ? <div className="flex justify-center py-12"><Loader2 className="size-5 animate-spin text-primary" /></div>
      : filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map(a => (
            <button
              key={a.id}
              onClick={() => { markRead(a.id); a.action_route && navigate(a.action_route); }}
              className={`w-full text-left card-soft border-l-4 ${sevColor(a.severity)} p-3 ${a.read ? "opacity-60" : ""}`}
            >
