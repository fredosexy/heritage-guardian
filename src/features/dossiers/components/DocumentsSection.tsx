...
export function DocumentsSection({ dossierId, bienId }: { dossierId: string; bienId?: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    setDocuments(items); setCoverage(currentCoverage);
  }, [dossierId]);
  ...
  return <section id="documents" className="card-soft p-5 mb-4 space-y-4">
    ...
    {documents.length === 0 ? <p className="text-caption text-center py-3">{t("documents.empty")}</p> : <div className="space-y-2">{documents.map((document) => <div key={document.id} className="rounded-lg border p-3"><div className="flex justify-between gap-2"><p className="text-sm font-medium truncate">{document.title}</p><span className="text-xs text-muted-foreground">{t(`documents.statuses.${document.verification_status}`)}</span></div><p className="text-caption">{t(`documents.types.${document.document_type}`, document.document_type)} · {t(`documents.sources.${document.source_type}`)}</p></div>)}</div>}
  </section>;
}
